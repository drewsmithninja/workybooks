...
import { browseByCommonCoreSubject, browseBySubject, subjectData, subjectCommonData } from '../../utils/appData';
      ...
      <Row gutter={[16, 16]} className='container !mx-auto mt-[30px]'>
        <Col lg={12} xs={24}>
          <Typography.Title level={3} className='md:text-left text-center'>
            {subjectRec && subjectRec.subjectType === 'common' ? `Common Core Standards - Grade 3 - ${subjectRec.subjectName}` : `${subjectRec && subjectRec.subjectFullName} - Grade 3`}
          </Typography.Title>
        </Col>
        <Col lg={12} xs={24} className='text-center md:text-right'>
          <Input placeholder={`Search ${subjectRec && subjectRec.subjectName} Topics`} className='w-full max-w-[487px] h-[40px] rounded-[60px]' suffix={<SearchOutlined className='text-[#A5A5A5]' />} />
        </Col>
      </Row>

      <Row gutter={[16, 16]} className='container !mx-auto mt-[30px]'>
        {subjectRec &&
          subjectRec.subjectType === 'subject' &&
          subjectDetails.subjectTopic.map((item, index) => (
            <Col span={24} key={Math.random()}>
              ...
              <Card title={false} className='mt-[10px] bg-gray-100 mb-[65px]'>
                <Row gutter={[16, 16]}>
                  {subjectDetails.subjectTopic[index].subTopics.length > 0 &&
                    subjectDetails.subjectTopic[index].subTopics.map((item1, index1) => (
