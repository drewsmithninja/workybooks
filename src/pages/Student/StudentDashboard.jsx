        ...
        <Space size="large">
          ...
          <div className="flex">
            <ADButton
              type="text"
              className="!p-0"
              onClick={() => showEditStudentModal(currentStudent)}>
                  ...
                  <Row gutter={16} className="w-full">
                    <Col xs={24} md={24} lg={12} xl={10} xxl={8}>
                      <ADImage
                        alt="cover-img"
                        src={dummyImage}
                        className="w-full aspect-[80px/100px] object-cover rounded max-w-[100px]"
                      />
                    </Col>
                    <Col xs={24} md={24} lg={12} xl={14} xxl={16} className="inter-font text-sm">
                      <div className="flex flex-col justify-center h-full lg:py-0 py-4">
                        ...
                        <div className="font-normal text-gray-400 truncate">
                          {item?.SubmittedAssignments?.contentScore?.content?.[0]?.stds_topic.map(
                            (topic, index) => (
                              <span key={index}>{`${topic}${
                                index < item?.contentScore?.content?.[0]?.stds_topic.length - 1
                                  ? ', '
                                  : ''
                              }`}</span>
                            )
                ...
                <Col
                  ...
                  <Row className="rounded-2xl md:px-4 px-2 py-4 border border-solid border-slate-300 w-full">
                    <Col
                      ...
                      <div>TIME</div>
                      <div>
                        {item?.submittedDate ? moment(item?.submittedDate).format('hh:mm') : '-'}
                      </div>
                    </Col>
                    <Col
                      xs={12}
