...
function AssignmentDetailsPage() {
  ...
  const { studentAssignmentDetail, studentAssignmentReportJson, assignments, isLoading } =
  ...
  const [currentSelectedAssignment, setCurrentSelectedAssignment] = useState(
    updatedAssignmentList?.[getIndex] || {}
  );
  useEffect(() => {
  ...
  const { assignmentDetails, assignmentItems, assignmentScore } =
    studentAssignmentDetail?.studentsAssignmentData?.[0];

  const viewAssignmentReportModal = (
    ...
    <MainLayout>
            ...
            <Space>
              ...
              {assignmentItems.map((item, index) => (
                <ADImage
                  ...
                  alt="Worksheet image"
                  className="flex bg-slate-300 w-[60px] h-[60px] rounded-xl aspect[1/1] mt-4"
                />
              ))}
      ...
      <div className="flex py-4 justify-between px-4 items-center">
        ...
        <Space size="large">
          ...
          <CsvDownloadButton
            data={studentAssignmentReportJson}
                    ...
                    <Row gutter={16} className="w-full">
                      <Col xs={24} md={24} lg={12} xl={10} xxl={8}>
                        <ADImage
                          ...
                          className="aspect-[80px/100px]   rounded max-w-[80px]"
                        />
                      </Col>
                    ...
                    <Row className="rounded-2xl md:px-4 px-2 py-4 border border-solid border-slate-300 w-full">
                      <Col
                        ...
                        <div className="font-medium text-lg">
                          {item?.time.length > 0 ? moment(item?.time?.[0]).format('hh:mm') : 0}
                        </div>
                      </Col>
                      <Col
                        xs={12}
                      ...
                      <Col
                        ...
                        <Progress
                          type="circle"
                          width={50}
                          percent={item?.averagePercentage}
                          status="none"
                        />
                      </Col>
                    </Row>
