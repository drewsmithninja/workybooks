      ...
      <Row gutter={16} className="w-full">
                ...
                <div className="font-normal text-gray-400">
                  {item?.content?.[0]?.stds_topic?.map((topic, index) => (
                    <span key={topic}>{`${topic}${
                      index < item?.content?.[0]?.stds_topic?.length - 1 ? ', ' : ''
                    }`}</span>
                  ))}
                </div>
              </div>
          ...
          <div className="whitespace-pre-wrap">
            {item?.assignedTo && item?.assignedTo === 'Classroom'
              ...
              ? item?.assignedStudents
                  ?.slice(0, 3)
                  .map((student, index) => (
                    <span key={student?._id}>{`${student?.fullName}${
                      index < item?.assignedStudents.slice(0, 3)?.length - 1 ? ', ' : ''
                    }`}</span>
                  ))
              : item?.assignedStudents?.map((student, index) => (
                  <span key={student?._id}>{`${student?.fullName}${
                    index < item?.assignedStudents.length - 1 ? ', ' : ''
                  }`}</span>
                ))}
        ...
        <Col xl={3} md={4} sm={3} xs={3} className="flex justify-center items-center">
          <ADButton
            ...
            className="flex text-4xl text-slate-400">
            <BsArrowRightCircle />
          </ADButton>
        </Col>
      </Row>
