...
function GradeComponent({ activeGrade = '1' }) {
  return (
    <div className='flex items-center justify-center pt-[20px]'>
      ...
      <Radio.Group defaultValue={activeGrade} size='small' buttonStyle='solid' className='h-[60px] sm:h-auto flex flex-wrap sm:!max-w-full max-w-[320px] items-center justify-center'>
        {grades.map((item, index) => (
          <Radio.Button value={`${item}`} key={`${item}`} className='mr-[10px] w-[58px] h-[25px] bg-[#D9D9D9] !rounded-[60px] text-xs text-center border-0 mb-[0px] sm:mb-0'>
            {item}
          </Radio.Button>
        ))}
      </Radio.Group>
