...
export default function ManualClassCreated({ onOk }) {
  ...
  return (
    <div className='flex flex-col items-center'>
      ...
      {newStudents?.student ? <div className='py-4 text-dark text-lg my-16 text-center'>{!newStudents || (classLoading && studentsLoading) ? <Spinner /> : `Classroom ( ${newStudents?.student?.class} ) with ( ${newStudents?.student?.students} ) ${newStudents?.student?.students === 1 ? ' student' : 'students'} has been created`}</div> : <div className='py-4 text-dark text-lg my-16 text-center'>{!newStudents ? <Spinner /> : `Classroom ( ${newStudents?.classroomName} ) with ( ${newStudents?.importedCount} ) ${newStudents?.importedCount === 1 ? ' student' : 'students'} has been created`}</div>}
